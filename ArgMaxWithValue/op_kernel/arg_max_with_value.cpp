...
public:
                                ...
                                uint32_t bigCoreNum, uint32_t bigCoreProcessNum, uint32_t smallCoreNum, uint32_t smallCoreProcessNum, uint32_t singleProcessNum, uint32_t bigTileNum, uint32_t bigTailNum, uint32_t smallTileNum, uint32_t smallTailNum)
    {
        ...
        pipe.InitBuffer(inQueueX, BUFFER_NUM, this->singleProcessNum * ((reduceAxisLen * subTensorLen * sizeof(half) + 31) / 32 * 32));
        pipe.InitBuffer(outQueueY, BUFFER_NUM, this->singleProcessNum * ((subTensorLen * sizeof(int32_t) + 31) / 32 * 32));
        pipe.InitBuffer(outQueueZ, BUFFER_NUM, this->singleProcessNum * ((subTensorLen * sizeof(half) + 31) / 32 * 32));
        pipe.InitBuffer(workQueue1, BUFFER_NUM, this->singleProcessNum * ((reduceAxisLen * sizeof(uint32_t) + 31) / 32 * 32));
        pipe.InitBuffer(workQueue2, BUFFER_NUM, this->singleProcessNum * ((reduceAxisLen * sizeof(half) + 31) / 32 * 32));
        pipe.InitBuffer(workQueue3, BUFFER_NUM, this->singleProcessNum * ((reduceAxisLen * sizeof(half) + 31) / 32 * 32));
        pipe.InitBuffer(workQueue4, BUFFER_NUM, this->singleProcessNum * ((reduceAxisLen * sizeof(half) + 31) / 32 * 32));
    }
...
private:
    __aicore__ inline void CopyIn(int32_t progress)
    {
        ...
        DataCopyPad(x_local, xGm[progress * this->reduceAxisLen * this->subTensorLen * this->curProcessNum], copyParams, padParams);
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
        ...
        {
            // x_local += j * (reduceAxisLen * subTensorLen * sizeof(half) + 31) / 32 * 32 / sizeof(half);
            // y_local += j * (subTensorLen * sizeof(int32_t) + 31) / 32 * 32 / sizeof(int32_t);
            // z_local += j * (subTensorLen * sizeof(half) + 31) / 32 * 32 / sizeof(half);
            // off_local += j * (reduceAxisLen * sizeof(uint32_t) + 31) / 32 * 32 / sizeof(uint32_t)sizeof(uint32_t);
            // src_local += j * (reduceAxisLen * sizeof(half) + 31) / 32 * 32 / sizeof(half);
            // dst_local += j * (reduceAxisLen * sizeof(half) + 31) / 32 * 32 / sizeof(half);
            // work_local += j * (reduceAxisLen * sizeof(half) + 31) / 32 * 32 / sizeof(half);
            ...
            {
                Gather(src_local, x_local[j * ((reduceAxisLen * subTensorLen * sizeof(half) + 31) / 32 * 32 / sizeof(half))], off_local, (uint32_t)(i * sizeof(half)), reduceAxisLen);
                PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline void CopyOut(int32_t progress)
    {
        ...
        uint32_t copysz2 = this->subTensorLen * sizeof(half) * this->curProcessNum;
        ...
        DataCopyPad(yGm[progress * this->subTensorLen * this->curProcessNum], y_local, copyParams1);
        DataCopyPad(zGm[progress * this->subTensorLen * this->curProcessNum], z_local, copyParams2);
