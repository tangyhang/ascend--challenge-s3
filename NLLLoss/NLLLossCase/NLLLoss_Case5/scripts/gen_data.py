...
def gen_golden_data_simple():    
    ...
    input_x = np.random.uniform(-5, 5,[1000,33] ).astype(test_type)
    input_target = np.random.uniform(0,32,[1000] ).astype(target_type)
    input_weight= np.random.uniform(0,1,[33] ).astype(test_type)
    reduction="sum";
    ignore_index=-100;
