...
def gen_golden_data_simple():    
    ...
    input_x = np.random.uniform(-5, 5,[543] ).astype(test_type)
    input_target = np.random.uniform(0,543,[543] ).astype(target_type)
    input_weight= np.random.uniform(0,1,[543] ).astype(test_type)
    reduction="sum";
    ignore_index=-100;
